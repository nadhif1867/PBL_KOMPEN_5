...
class dDikerjakanOlehController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('dDikerjakanOleh.index', ['breadcrumb' => $breadcrumb, 'page' => $page, 'activeMenu' => $activeMenu]);
    }

    public function list(Request $request)
    {
        // Ambil data  status_penerimaan = 'request'
        $dManageKompens = TugasKompenModel::select('id_tugas_kompen', 'id_mahasiswa', 'id_tugas_dosen', 'status_penerimaan')
            ->where('status_penerimaan', 'request')
            ->whereNotNull('id_tugas_dosen')
            ->with('mahasiswa')
            ->with('tugasdosen');
        ...
        return DataTables::of($dManageKompens)
            ...
            ->addColumn('aksi', function ($dManageKompen) {
                $btn = '<button onclick="updateStatus(' . $dManageKompen->id_tugas_kompen . ', \'diterima\')" class="btn btn-success btn-sm" style="margin-top: 5px;">Diterima</button>';
                $btn .= '<button onclick="updateStatus(' . $dManageKompen->id_tugas_kompen . ', \'ditolak\')" class="btn btn-danger btn-sm" style="margin-top: 5px;">Tolak</button>';
                return $btn;
            })
            ->rawColumns(['aksi'])
            ->make(true);
